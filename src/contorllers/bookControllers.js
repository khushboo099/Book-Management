...
const bookCreation = async function (req, res) {
    try {
        ...
        if (!Validator.isISBN(ISBN)) return res.status(400).send({ status: false, message: "invalid ISBN" })
        ...
        if (findISBN) return res.status(400).send({ status: false, message: "ISBN already exist with another book" });
        ...
        data["releasedAt"] =new Date().toLocaleDateString()
        const creatbook = await bookModel.create(data)
        return res.status(201).send({ status: true, message: "Book created successfully", data: creatbook })
    }
    catch (err) {
