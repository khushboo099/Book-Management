...
const createUser = async (req, res) => {
    try {
        ...
        if (numberexist)
            return res.status(409).send({ status: false, message: `This ${phone} number  is already registered` })
        ...
        if (emailexist.length != 0)
            return res.status(409).send({ status: false, message: `This ${emailId} email  is already registerd` })
        ...
        if (!validator.isNumeric(address.pincode))
            return res.status(400).send({ status: false, message: "The entered pincode should be number" })
