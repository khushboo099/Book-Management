...
const loginUser = async (req, res) => {
    ...
    if (!userExist) return res.status(401).send({ status: false, message: "this email is not registered" });
    //------------pasword validation------------//
    if (!isValid(password))
        return res
            .status(400)
    ...
    if (!passwordValid) return res.status(401).send({ status: false, message: "this password is not matching with your email id" });
     let token = jwt.sign({
