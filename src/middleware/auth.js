...
const bookCheck =  async (req,res,next)=>{
   try {const bookIds=req.params.bookId
     const book=await bookModel.findById(bookIds)
        if((!book )|| book.isDeleted) return res.status(404).send(`book is not present in the data base`)
        if(!isValidObjectId(bookIds)) res.status(400).send({status:false,message:`the book id ${bookIds} not  valid `})
